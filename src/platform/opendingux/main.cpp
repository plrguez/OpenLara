...
vec2 joyL, joyR;
float joyVGainL, joyVGainR, joyVGainOldL, joyVGainOldR;
int   joyVTime;
...
InputKey codeToInputKey(int code) {
    ...
    // keyboard
        ...
        case SDL_SCANCODE_BACKSPACE  : return ikBack;
        ...
        case SDL_SCANCODE_PAGEUP     : return ikPrev;
        case SDL_SCANCODE_PAGEDOWN   : return ikNext;
        case SDL_SCANCODE_KP_DIVIDE  : return ikNDiv;
        case SDL_SCANCODE_KP_PERIOD  : return ikNDot;
...
bool inputInit() {
    ...
    if ( (Shake_Init() == SHAKE_ERROR) || (Shake_NumOfDevices() == 0) ) {
	LOG("! can't initialize vibration\n");
    } else {
	shake_device = Shake_Open(0);
	if (!shake_device) {
		LOG("! can't initialize shake device\n");
		Shake_Quit();
	} else if (!Shake_QueryEffectSupport(shake_device, SHAKE_EFFECT_RUMBLE) || !Shake_QueryGainSupport(shake_device)) {
		LOG("! can't initialize shake device\n");
		Shake_Close(shake_device);
		Shake_Quit();
		shake_device=0;
	} else {
	        Shake_SimpleRumble(&shake_effect,1.0,1.0,0.1);
	        shake_id = Shake_UploadEffect(shake_device, &shake_effect);
	}
    }
...
void inputFree() {
    ...
    if (shake_device) {
	Shake_EraseEffect(shake_device, shake_id);
	Shake_Close(shake_device);
	Shake_Quit();
    }
}
...
void joyRumble() {
    if ( shake_device == 0 || ( joyVGainL == 0 && joyVGainR == 0 && joyVGainOldL == 0 && joyVGainOldR == 0 ) || osGetTimeMS() < joyVTime )
        return;

    if (joyVGainL != 0.0f || joyVGainR != 0.0f) {
	shake_effect.id                       = shake_id;
	shake_effect.u.rumble.strongMagnitude = int(joyVGainL * 65535);
	shake_effect.u.rumble.weakMagnitude   = int(joyVGainR * 65535);
	shake_effect.length                   = int(max(JOY_MIN_UPDATE_FX_TIME, 1000.0f / Core::stats.fps));
	shake_id = Shake_UploadEffect(shake_device, &shake_effect);
	if (shake_id == SHAKE_ERROR) {
		return;
	}
        Shake_Play(shake_device, shake_id);
    } else if (joyVGainL == 0.0f && joyVGainR == 0.0f) {
        Shake_Stop(shake_device, shake_id);
    }

    joyVGainOldL = joyVGainL;
    joyVGainOldR = joyVGainR;
